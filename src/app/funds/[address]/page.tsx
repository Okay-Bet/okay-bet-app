...
export default function FundDetailPage() {
  ...
  const loadFundDetails = async () => {
    ...
    try {
      ...
      // Load source group information
      try {
        const groupsResponse = await fetch('/api/groups?limit=100');
        const groupsData = await groupsResponse.json();

        if (groupsResponse.ok && groupsData?.groups) {
          const sourceGroup = groupsData.groups.find((g: any) => {
            const metadata = g.metadata;
            return metadata?.fund_deployment?.contract_address?.toLowerCase() === fundAddress.toLowerCase();
          });

          if (sourceGroup) {
            setSourceGroupId(sourceGroup.id);
            setSourceGroupName(sourceGroup.title);
          }
        }
      } catch (err) {
        console.log('Could not load source group:', err);
            ...
            <div>
              ...
              {sourceGroupId && sourceGroupName && (
                <div className="mt-2 mb-3 flex items-center gap-3">
                  <a
                    href={`/groups`}
                    className="inline-flex items-center gap-1 text-sm text-primary hover:text-primary/80 transition font-medium"
                  >
                  ...
                  <a
                    href={`/groups/${sourceGroupId}/trading-instructions`}
                    className="inline-flex items-center gap-1 text-sm text-primary hover:text-primary/80 transition font-medium"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
