...
export default function FundsPage() {
  ...
  const [viewMode, setViewMode] = useState<'active-funds' | 'deploy-new'>('deploy-new');
  ...
  const [selectedAgent, setSelectedAgent] = useState<'pamela' | 'chalk-eater'>('pamela');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        {loading ? (
          ...
          groupsWithoutFunds.length === 0 ? (
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* Special tile for automated strategy without group */}
                <div className="bg-gradient-to-br from-red-50 to-black/10 rounded-lg shadow-md p-6 border-2 border-red-600 hover:border-red-700 transition">
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      <svg className="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <h3 className="text-lg font-bold text-gray-900">Automated Strategy</h3>
                    </div>
                    <p className="text-sm text-gray-600 line-clamp-2">
                      Deploy an AI agent with fully automated trading strategy. No group required - the agent manages positions independently.
                    </p>
                  </div>
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm items-center">
                      ...
                      <select
                        value={selectedAgent}
                        onChange={(e) => setSelectedAgent(e.target.value as 'pamela' | 'chalk-eater')}
                        className="px-3 py-1 border border-gray-300 rounded-lg text-sm font-semibold text-gray-900 bg-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
                      >
                        <option value="pamela">Pamela</option>
                        <option value="chalk-eater">Chalk Eater</option>
                      </select>
                    </div>
      ...
      {/* Create Fund Modal for Automated Strategy */}
      <CreateFundModal
        isOpen={showAutomatedModal}
        onClose={() => setShowAutomatedModal(false)}
        groupName={`Automated ${selectedAgent === 'pamela' ? 'Pamela' : 'Chalk Eater'} Strategy`}
        hideAgentSection={true}
        onSuccess={(fundAddress) => {
          console.log('Automated fund created:', fundAddress);
          setShowAutomatedModal(false);
          setViewMode('active-funds');
          loadFunds();
        }}
      />
