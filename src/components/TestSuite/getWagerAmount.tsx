...
const BET_ABI = [
  ...
  const WagerAmountReader = () => {
    ...
    const [makerWager, setMakerWager] = useState(null);
    const [takerWager, setTakerWager] = useState(null);
    ...
    return (
      <div className="p-4 bg-gray-100 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p><strong>Maker:</strong> {betDetails.maker}</p>
            <p><strong>Taker:</strong> {betDetails.taker}</p>
            <p><strong>Total Wager:</strong> {betDetails.totalWager} ETH</p>
            <p><strong>Wager Ratio:</strong> {betDetails.wagerRatio}%</p>
          </div>
          <div>
            <p><strong>Status:</strong> {betDetails.status}</p>
            <p><strong>Finalized:</strong> {betDetails.finalized ? 'Yes' : 'No'}</p>
            <p><strong>Wager Currency:</strong> {betDetails.wagerCurrency === ethers.constants.AddressZero ? 'ETH' : betDetails.wagerCurrency}</p>
          </div>
        </div>
        <div className="mt-4">
          <h3 className="text-lg font-semibold">Wager Amounts:</h3>
          <p><strong>Maker's Wager:</strong> {makerWager ? `${makerWager} ETH` : 'Loading...'}</p>
          <p><strong>Taker's Wager:</strong> {takerWager ? `${takerWager} ETH` : 'Loading...'}</p>
        </div>
      </div>
    );
  };
